...
- options with values (e.g. --witness <path>)
...
/-- Parse CLI args into Options. Accepts arbitrary ordering of flags.
    ...
    positional tokens (if present) are interpreted as input and output
    paths respectively, unless overridden by explicit options. -/
  ...
  let rec go (acc : Options) (positional : List String) (rem : List String) : Except String Options :=
    ...
    | [] =>
      ...
      let acc := match positional with
        | [] => acc
        | [a] => if acc.cmd == "" then { acc with cmd := a } else if acc.input?.isNone then { acc with input? := some a } else acc
        | [a, b] =>
          let acc := if acc.cmd == "" then { acc with cmd := a } else if acc.input?.isNone then { acc with input? := some a } else acc
          let acc := if acc.output?.isNone then { acc with output? := some b } else acc
          acc
        | a :: b :: rest =>
          -- More than two positionals: treat first as cmd, next as input, next as output; ignore extras
          let acc := if acc.cmd == "" then { acc with cmd := a } else acc
          let acc := if acc.input?.isNone then { acc with input? := some b } else acc
